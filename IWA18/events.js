...
const handleDragOver = (event) => {
  ...
  if (!column) return;
  dataFile.updateDragging({ over: column });
  viewFile.updateDraggingHtml({ over: column });
};
...
for (const htmlColumn of Object.values(viewFile.html.columns)) {
  htmlColumn.addEventListener('dragstart', handleDragStart);
  htmlColumn.addEventListener('dragend', handleDragEnd);
}

for (const htmlArea of Object.values(viewFile.html.area)) {
  htmlArea.addEventListener('dragover', handleDragOver);
}
